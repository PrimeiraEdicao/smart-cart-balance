// src/components/CategoryManagerDialog.tsx
...
export const CategoryManagerDialog = ({ open, onOpenChange }: CategoryManagerDialogProps) => {
  ...
  const handleAddCategory = () => {
    if (newCategoryName.trim()) {
      ...
      setNewCategoryName('');
      setNewCategoryColor(colorOptions[0]); // Reset para cor padrão
    }
  };
  ...
  const handleSaveEdit = () => {
    if (editingId && editingName.trim()) {
      ...
      if (categoryToUpdate) {
        updateCategory({ ...categoryToUpdate, name: editingName.trim() });
      }
    }
    handleCancelEdit();
  };
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {colorOptions.map(color => (
                <button 
                  ...
                  className={`w-6 h-6 rounded-full transition-transform hover:scale-110 ${newCategoryColor === color ? 'ring-2 ring-offset-2 ring-blue-500' : ''}`} 
